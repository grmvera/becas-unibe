...
@Component({
  ...
  `,
  styles: [`
    .dialog-title { margin: 0; padding: 16px 24px; font-weight: 500; font-size: 20px; border-bottom: 1px solid #e0e0e0; }
    .dialog-body { padding: 16px 24px; box-sizing: border-box; }
    .form-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; }
    .col-span { grid-column: span 2; }
    .estado-box { display: flex; align-items: center; padding-left: 8px; }
    .acciones { grid-column: span 2; display: flex; justify-content: flex-end; gap: 8px; margin-top: 8px; }
    @media (max-width: 600px) {
      .form-grid { grid-template-columns: 1fr; }
      .col-span, .acciones { grid-column: span 1; }
    }
  `]
})
export class AsignarPorcentajeDialogComponent {
  ...
  async prefill(carreraId: string) {
    ...
    if (snap.exists()) {
      ...
      // Opcional: prellenar y avisar que ya está asignada
      this.form.patchValue({
        porcentaje: typeof data.porcentaje === 'number' ? data.porcentaje : null,
        estado: typeof data.estado === 'boolean' ? data.estado : true
      });
      this.snack.open(
        `Esta carrera ya tiene asignado ${data.porcentaje}% de beca. Para reasignar, primero elimina la asignación anterior.`,
        'Entendido',
        { duration: 4500 }
      );
    } else {
  ...
  async guardar() {
    ...
    try {
      ...
      // 3) VALIDACIÓN: ¿ya existe asignación para esta carrera?
      const asignRef = doc(this.fs, 'porcentajes_carrera', carreraId);
      const prevSnap = await getDoc(asignRef);
      if (prevSnap.exists()) {
        const prev = prevSnap.data() as any;
        this.snack.open(
          `La carrera "${carreraNombre}" ya tiene asignado ${prev?.porcentaje ?? '?'}%. ` +
          `Elimina la asignación anterior para poder crear una nueva.`,
          'Cerrar',
          { duration: 5500 }
        );
        this.cargando = false;
      ...
      // 4) crear (no sobrescribe porque validamos arriba)
      const now = Date.now();
